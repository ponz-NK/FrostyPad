const { app, BrowserWindow, globalShortcut, ipcMain, shell, screen, dialog, Menu } = require('electron');
const path = require('path');
const fs = require('fs').promises; // Use promises for easier async/await
const fsSync = require('fs'); // Sync fs for close event

// Helper to handle paths in Dev vs Prod (EXE)
function getConfigPath(filename) {
    if (!app.isPackaged) {
        // In Dev: Use project root (parent of src)
        return path.join(__dirname, '..', filename);
    }
    // In Prod: Use UserData folder (AppData/Roaming/...)
...
const DEFAULT_SETTINGS = {
    ...
    backgroundImage: "assets/images/settings-bg-default.png", // settings-bg-default.pngを使用
    enableFolderRename: true,
    enableAppRename: false,
    desktopNameColor: '#ffffff',
    desktopNameBold: false, // white off -> bold off? (text style white off usually implies color or weight, assuming user meant bold off based on previous context, and color white above)
    showSearch: false,
    showScroll: false,
    windowLocked: false,
    compactMenu: false // Compact menu off
};
...
async function ensureDataFiles() {
    ...
    const appsPath = getConfigPath('apps.json');
...
ipcMain.handle('load-config', async () => {
    try {
        const configPath = getConfigPath('apps.json');
