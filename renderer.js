...
// ... createAppElement definition will be updated later ...
...
async function createAppElement(app, context = 'desktop', contextIndex = -1, parentIndex = -1) {
    ...
    if (app.type === 'folder') {
        ...
        // We need to resolve icons for sub-apps too
        const subApps = app.content.slice(0, 9);
        for (const subApp of subApps) {
             const mini = document.createElement('div');
            mini.className = 'mini-icon';
            // Try to load icon for mini preview
            if (window.electronAPI && subApp.path) {
                try {
    ...
    // RENAMING LOGIC (Applied always if permission allows)
    // PERMISSIONS CHECK
    const canEdit = (app.type === 'folder' && currentSettings.enableFolderRename !== false) || 
                    (app.type === 'app' && currentSettings.enableAppRename === true);

    if (canEdit) {
        name.onclick = (e) => {
            e.preventDefault();
            e.stopPropagation(); // Prevent launch, start editing
            
            // Only start editing if not already editing?
            // Simple approach: make contentEditable true on click
        ...
        name.draggable = true; 
        name.ondragstart = (e) => {
             // If we are editing, stop drag?
             // Or if we are just clicking text to rename, we don't want to drag the app.
             e.preventDefault();
             e.stopPropagation();
        };
        ...
        // Not editable
        name.onclick = (e) => {
             // If not editable, clicking name should launch app (bubble up to el.onclick) or do nothing?
             // Usually on typical OS, clicking name selects it. Here launch is on parent.
             // If we stop propagation here, we disable launch via text.
             // If we allow propagation, it launches.
             // Requirement says "click name to rename IF allowed". 
             // If NOT allowed, behavior wasn't specified, but fallback to launch (default bubbling) is logical.
             // So we do NOTHING here, letting it bubble to el.onclick.
        };
    }
...
function applySettings(settings) {
    ...
    if (settings.backgroundImage) {
        ...
        if (!url.startsWith('http') && !url.startsWith('file://')) {
            // Assume local path, escape backslashes
...
if (window.electronAPI) {
    window.electronAPI.on('settings-updated', (newSettings) => {
        ...
        applySettings(currentSettings);
        renderApps(); // Re-render in case icon size/gaps changed layout
    });
}
